...
fleetcdir:
  file.directory:
    - name: /opt/so/conf/fleetkube/etc
    - user: 939
    - group: 939
    - makedirs: True
...
fleetpackcdir:
  file.directory:
    - name: /opt/so/conf/fleetkube/packs
    - user: 939
    - group: 939
    - makedirs: True
...
fleetpacksync:
  file.recurse:
    - name: /opt/so/conf/fleetkube/packs
    - source: salt://fleetkube/files/packs
    - user: 939
    - group: 939

fleetpackagessync:
  file.recurse:
    - name: /opt/so/conf/fleetkube/packages
    - source: salt://fleetkube/packages/
    - user: 939
    - group: 939

fleetlogdir:
  file.directory:
    - name: /opt/so/log/fleetkube
    - user: 939
    - group: 939
    - makedirs: True
...
#    - image: {{ MANAGER }}:5000/{{ IMAGEREPO }}/so-fleet:{{ VERSION }}
    - image: {{ MANAGER }}:5000/keepwn/so-fleet:3.12.0-plus
    - hostname: so-fleetkube
    - port_bindings:
      - 0.0.0.0:8080:8080
    - environment:
...
append_so-fleetkube_so-status.conf:
  file.append:
